...
module.exports = function(formAction, questionType){
    ...
    htmlForm += form.open();
    htmlForm += 'Type ' + questionType + br + br;
        ...
        if (toGenerate.hasOwnProperty(property) && property!='type') {
            ...
            if(property == 'subject') {
                htmlForm += form.select()
                                .attr({name: property, required:'required', multiple: false})
                                .setOptions(subjects)
                ...
                if(questionType == 'Reponse a developpement') {
                    htmlForm += form.textarea().attr({name: property, required:'required', rows: 6}).render() + br;  
                } else {
                    htmlForm += form.text().attr({name: property, required:'required'}).render() + br; 
                }
            }
        }
    }
